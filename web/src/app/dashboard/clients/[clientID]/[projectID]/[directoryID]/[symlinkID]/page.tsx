...
import axios from "@train360-corp/dms/lib/supabase/axios";
...
export default async function Page(props: {
  ...
  if (("version" in searchParams) && (typeof searchParams.version !== "string" || Number.isNaN(Number(searchParams.version)))) {
  ...
  if (
    client.error || project.error || directory.error ||
    symlink.error || file.error || version.error || object.error
  ) {
    redirect("/dashboard/clients");
  }

  return (
    <div className="@container/main flex flex-col h-screen overflow-hidden">
      {/* Header Section */}
      ...
      {/* Scrollable Viewer Section */}
      <div className="flex-1 overflow-scroll px-4 pb-4 md:px-6 md:pb-6">
        <div className="h-full w-full rounded-lg border bg-background shadow">
          <FileViewer object={object.data} />
        </div>
      </div>
    </div>
  );
